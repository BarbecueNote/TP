...
enum Maze {
  ...
  def explore(trace: ListBuffer[String]): Unit = this match
    case branch@Branch(label, left, right, status) =>
      status match
        case Exploration.UnExplored => branch.status = Exploration.Explored; trace.append(label); left.explore(trace); right.explore(trace)
        case Exploration.Explored => trace.append(label)
    case Leaf(label) => trace.append(label)
}
@main
def main(): Unit = {
  // println("Hello world!")
  ...
  val trace = new ListBuffer[String]()
